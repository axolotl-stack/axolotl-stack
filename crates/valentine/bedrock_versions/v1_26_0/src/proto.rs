...
impl crate::bedrock::codec::BedrockCodec for GraphicsOverrideParameterPacket {
    ...
    fn decode<B: bytes::Buf>(
        ...
        let float_value =
            <crate::bedrock::codec::F32LE as crate::bedrock::codec::BedrockCodec>::decode(buf, ())?
                .0;
        let vec_3_value = <Vec3F as crate::bedrock::codec::BedrockCodec>::decode(buf, ())?;
...
impl crate::bedrock::codec::BedrockCodec for ServerboundDataStorePacket {
    ...
    fn encode<B: bytes::BufMut>(&self, buf: &mut B) -> Result<(), std::io::Error> {
        ...
        crate::bedrock::codec::I32LE(self.path_update_count).encode(buf)?;
        Ok(())
    }
    fn decode<B: bytes::Buf>(
        buf: &mut B,
...
impl crate::bedrock::codec::BedrockCodec for ClientboundDataDrivenUiShowScreenPacket {
    ...
    fn encode<B: bytes::BufMut>(&self, buf: &mut B) -> Result<(), std::io::Error> {
        ...
        let bytes: Vec<u8> = if "".eq_ignore_ascii_case("latin1") {
            ...
            (&self.screen_id).as_bytes().to_vec()
        };
        let len = bytes.len();
        crate::bedrock::codec::VarInt(len as i32).encode(buf)?;
        buf.put_slice(&bytes);
...
impl crate::bedrock::codec::BedrockCodec for ClientboundDataDrivenUiCloseAllScreensPacket {
    type Args = ();
    fn encode<B: bytes::BufMut>(&self, buf: &mut B) -> Result<(), std::io::Error> {
        let _ = buf;
        Ok(())
...
impl crate::bedrock::codec::BedrockCodec for ClientboundTextureShiftPacket {
    ...
    fn encode<B: bytes::BufMut>(&self, buf: &mut B) -> Result<(), std::io::Error> {
        ...
        let bytes: Vec<u8> = if "".eq_ignore_ascii_case("latin1") {
            ...
            (&self.collection_name).as_bytes().to_vec()
        };
        let len = bytes.len();
        crate::bedrock::codec::VarInt(len as i32).encode(buf)?;
        buf.put_slice(&bytes);
        let bytes: Vec<u8> = if "".eq_ignore_ascii_case("latin1") {
            ...
            (&self.from_step).as_bytes().to_vec()
        };
        let len = bytes.len();
        crate::bedrock::codec::VarInt(len as i32).encode(buf)?;
        buf.put_slice(&bytes);
