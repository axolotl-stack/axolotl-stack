...
impl crate::bedrock::codec::BedrockCodec for WindowIdVarint {
    ...
    fn decode<B: bytes::Buf>(buf: &mut B, _args: Self::Args) -> Result<Self, std::io::Error> {
        ...
        match val {
            ...
            248 => Ok(WindowIdVarint::CombinedInventory),
            // For unknown values, default to Inventory to avoid crashes
            _ => Ok(WindowIdVarint::Inventory),
        }
    }
}
    ...
    fn decode<B: bytes::Buf>(buf: &mut B, _args: Self::Args) -> Result<Self, std::io::Error> {
        ...
        match val {
            ...
            _ => Err(std::io::Error::new(
                ...
                format!("Invalid enum value for {}: {}", stringify!(WindowId), val),
            )),
        }
    }
}
...
impl crate::bedrock::codec::BedrockCodec for WindowType {
    ...
    fn decode<B: bytes::Buf>(buf: &mut B, _args: Self::Args) -> Result<Self, std::io::Error> {
        ...
        match val {
            ...
            _ => Err(std::io::Error::new(
                std::io::ErrorKind::InvalidData,
                format!("Invalid enum value for {}: {}", stringify!(WindowType), val),
            )),
        }
    }
}
