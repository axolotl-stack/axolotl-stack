...
/// TextPacket - Manual override matching gophertunnel's flat structure.
/// The generated version uses complex nested enums for StringConst fields.
/// This flat structure is easier to use and will be compatible when string enums are removed.
#[derive(Debug, Clone, PartialEq, Default)]
...
fn write_string<B: bytes::BufMut>(buf: &mut B, s: &str) -> Result<(), std::io::Error> {
...
fn read_string<B: bytes::Buf>(buf: &mut B) -> Result<String, crate::bedrock::error::DecodeError> {
    let len_raw = <crate::bedrock::codec::VarInt as crate::bedrock::codec::BedrockCodec>::decode(buf, ())?.0 as i64;
    if len_raw < 0 {
...
impl crate::bedrock::codec::BedrockCodec for TextPacket {
    ...
    fn encode<B: bytes::BufMut>(&self, buf: &mut B) -> Result<(), std::io::Error> {
        ...
        // 2. Category (computed from type)
        let category: u8 = match self.type_ {
            TextPacketType::Raw | TextPacketType::Tip | TextPacketType::System |
            TextPacketType::JsonWhisper | TextPacketType::JsonAnnouncement | TextPacketType::Json => 0, // MessageOnly
            TextPacketType::Chat | TextPacketType::Whisper | TextPacketType::Announcement => 1, // Authored
            TextPacketType::Translation | TextPacketType::Popup | TextPacketType::JukeboxPopup => 2, // Parameters
        };
        category.encode(buf)?;

        // 3. StringConst markers based on category
        match category {
            0 => { // MessageOnly
                write_string(buf, "raw")?;
                write_string(buf, "tip")?;
                write_string(buf, "systemMessage")?;
                write_string(buf, "textObjectWhisper")?;
                write_string(buf, "textObjectAnnouncement")?;
                write_string(buf, "textObject")?;
            }
            1 => { // Authored
                write_string(buf, "chat")?;
                write_string(buf, "whisper")?;
                write_string(buf, "announcement")?;
            }
            _ => { // Parameters
                write_string(buf, "translate")?;
                write_string(buf, "popup")?;
                write_string(buf, "jukeboxPopup")?;
            }
        }
        ...
        match self.type_ {
            ...
            TextPacketType::Raw | TextPacketType::Tip | TextPacketType::System |
            TextPacketType::Json | TextPacketType::JsonWhisper | TextPacketType::JsonAnnouncement => {
                write_string(buf, &self.message)?;
            }
            TextPacketType::Translation | TextPacketType::Popup | TextPacketType::JukeboxPopup => {
                write_string(buf, &self.message)?;
                crate::bedrock::codec::VarInt(self.parameters.len() as i32).encode(buf)?;
        ...
        // 6. XUID, PlatformChatID, FilteredMessage
        write_string(buf, &self.xuid)?;
        write_string(buf, &self.platform_chat_id)?;
        let has_filtered = self.filtered_message.is_some();
        has_filtered.encode(buf)?;
        if let Some(filtered) = &self.filtered_message {
            write_string(buf, filtered)?;
        }
    ...
    fn decode<B: bytes::Buf>(
        ...
        // 2. Category
        let category = <u8 as crate::bedrock::codec::BedrockCodec>::decode(buf, ())?;

        // 3. Skip StringConst markers based on category
        match category {
            0 => { // MessageOnly - 6 strings
                for _ in 0..6 { let _ = read_string(buf)?; }
            }
            1 => { // Authored - 3 strings
                for _ in 0..3 { let _ = read_string(buf)?; }
            }
            _ => { // Parameters - 3 strings
                for _ in 0..3 { let _ = read_string(buf)?; }
            }
        }
        ...
        // 5. Content based on type
        let (source_name, message, parameters) = match type_ {
            TextPacketType::Chat | TextPacketType::Whisper | TextPacketType::Announcement => {
                let source = read_string(buf)?;
                let msg = read_string(buf)?;
                (source, msg, Vec::new())
            }
            TextPacketType::Raw | TextPacketType::Tip | TextPacketType::System |
            TextPacketType::Json | TextPacketType::JsonWhisper | TextPacketType::JsonAnnouncement => {
                let msg = read_string(buf)?;
                (String::new(), msg, Vec::new())
            }
            TextPacketType::Translation | TextPacketType::Popup | TextPacketType::JukeboxPopup => {
                let msg = read_string(buf)?;
                let param_count = <crate::bedrock::codec::VarInt as crate::bedrock::codec::BedrockCodec>::decode(buf, ())?.0 as usize;
                let mut params = Vec::with_capacity(param_count);
                for _ in 0..param_count {
                    params.push(read_string(buf)?);
                }
                (String::new(), msg, params)
            }
        };
        ...
        let has_filtered = <bool as crate::bedrock::codec::BedrockCodec>::decode(buf, ())?;
