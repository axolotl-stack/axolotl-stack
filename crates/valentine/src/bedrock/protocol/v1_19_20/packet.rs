...
impl crate::bedrock::codec::BedrockCodec for PacketFeatureRegistry {
    fn encode<B: bytes::BufMut>(&self, buf: &mut B) -> Result<(), std::io::Error> {
        let len = self.features.len() as i32;
        len.encode(buf)?;
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
#[repr(u8)]
pub enum PacketModalFormResponseContentSomeCancelReason {
    Closed = 0,
    Busy = 1,
}
impl crate::bedrock::codec::BedrockCodec
for PacketModalFormResponseContentSomeCancelReason {
    fn encode<B: bytes::BufMut>(&self, buf: &mut B) -> Result<(), std::io::Error> {
        let val = *self as u8;
        val.encode(buf)
    }
    fn decode<B: bytes::Buf>(buf: &mut B) -> Result<Self, std::io::Error> {
        ...
        match val {
            0 => Ok(PacketModalFormResponseContentSomeCancelReason::Closed),
            1 => Ok(PacketModalFormResponseContentSomeCancelReason::Busy),
            _ => {
                Err(
                    std::io::Error::new(
                        std::io::ErrorKind::InvalidData,
                        format!(
                            ...
                            stringify!(PacketModalFormResponseContentSomeCancelReason),
                            val
                        ),
                    ),
                )
...
#[derive(Debug, Clone, PartialEq)]
pub struct PacketModalFormResponseContentSome {
    pub cancel_reason: PacketModalFormResponseContentSomeCancelReason,
}
impl crate::bedrock::codec::BedrockCodec for PacketModalFormResponseContentSome {
    fn encode<B: bytes::BufMut>(&self, buf: &mut B) -> Result<(), std::io::Error> {
        self.cancel_reason.encode(buf)?;
        Ok(())
    }
    fn decode<B: bytes::Buf>(buf: &mut B) -> Result<Self, std::io::Error> {
        let cancel_reason = <PacketModalFormResponseContentSomeCancelReason as crate::bedrock::codec::BedrockCodec>::decode(
            buf,
        )?;
...
pub struct PacketModalFormResponse {
    ...
    pub content: Option<PacketModalFormResponseContentSome>,
}
impl crate::bedrock::codec::BedrockCodec for PacketModalFormResponse {
    fn encode<B: bytes::BufMut>(&self, buf: &mut B) -> Result<(), std::io::Error> {
        self.form_id.encode(buf)?;
        self.has_response_data.encode(buf)?;
        if let Some(v) = &self.data {
            v.encode(buf)?;
        }
        self.has_cancel_reason.encode(buf)?;
    ...
    fn decode<B: bytes::Buf>(buf: &mut B) -> Result<Self, std::io::Error> {
        ...
        let data = match has_response_data {
        ...
        let content = match has_cancel_reason {
            true => {
                Some(
                    <PacketModalFormResponseContentSome as crate::bedrock::codec::BedrockCodec>::decode(
                        buf,
                    )?,
                )
            }
