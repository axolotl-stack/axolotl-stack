...
use crate::valentine::{
    EnumsServerboundLoadingScreenPacketType, McpePacket, McpePacketData, McpePacketName,
    NetworkSettingsPacketCompressionAlgorithm,
};
...
impl<T: Transport> BedrockStream<Login, Client, T> {
    ...
    pub async fn send_login(
        ...
        let login_pkt = LoginPacket {
            client_network_version: crate::valentine::PROTOCOL_VERSION,
            connection_request: crate::auth::client::encode_login_request(&chain, &client_token),
        };
        self.transport
...
mod tests {
    ...
    fn start_game_packet() -> McpePacket {
        McpePacket::from(StartGamePacket {
            runtime_id: crate::valentine::ActorRuntimeId {
                actor_runtime_id: 42,
            },
            ..Default::default()
        })
    ...
    async fn start_game_caps_aggregate_deferred_frame_bytes() {
        ...
        let level_chunk = || {
            McpePacket::from(crate::valentine::LevelChunkPacket {
                serialized_chunk_data: vec![0; HALF_LIMIT],
                ..Default::default()
            })
        };
...
impl<T: Transport> BedrockStream<ResourcePacks, Client, T> {
    ...
    pub async fn handle_packs(
        ...
        if let McpePacketData::ResourcePacksInfoPacket(ref info) = info_pkt.data {
            tracing::debug!(
                "ResourcePacksInfo: required={}, resource_packs={}",
        ...
        if let McpePacketData::ResourcePackStackPacket(ref stack) = stack_pkt.data {
            tracing::debug!(
                "ResourcePackStack: required={}, game_version={}, resource_packs={}",
        ...
        loop {
            ...
            match raw.id {
                McpePacketName::StartGamePacket => {
                    ...
                    let start_runtime_id = start.runtime_id.actor_runtime_id;
                    tracing::debug!(runtime_id = %start_runtime_id, "StartGame received");
                    if let Some(existing) = runtime_entity_id {
                        if existing != start_runtime_id {
                            return Err(ProtocolError::UnexpectedHandshake(format!(
                        ...
                        runtime_entity_id = Some(start_runtime_id);
        ...
        self.transport
            .send_batch(&[
                ...
                McpePacket::from(SetLocalPlayerAsInitializedPacket {
                    player_id: crate::valentine::ActorRuntimeId {
