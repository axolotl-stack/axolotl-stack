    ...
    pub(super) fn break_block(&mut self, breaking_player: Entity, x: i32, y: i32, z: i32) {
        ...
        if original_block_id != *blocks::AIR {
            ...
            if is_survival {
                ...
                let item_network_id = {
                    ...
                    let block_name = BLOCKS.get(original_block_id as usize)
                        .map(|b| b.name());
                    
                    use crate::registry::RegistryEntry;
                    block_name.and_then(|name| {
                        self.items.get_by_name(name).map(|item| item.id())
                    }).unwrap_or(0)
                };
                ...
                if item_network_id > 0 {
                    ...
                    use jolyne::protocol::packets::PacketAddItemEntity;
                    use jolyne::protocol::types::item::{Item, ItemContent, ItemContentExtra, ItemExtraDataWithoutBlockingTick};

                    let item_packet = PacketAddItemEntity {
                        ...
                        item: Item {
                            ...
                            content: Some(Box::new(ItemContent {
                                ...
                                extra: ItemContentExtra::Default(ItemExtraDataWithoutBlockingTick::default()),
                            })),
                        },
                        position: Vec3F {
                            x: x as f32 + 0.5,
        ...
        if let Some(chunk_viewers) = world.get::<ChunkViewers>(chunk_entity) {
            ...
            let particle_packet = PacketLevelEvent {
                event: PacketLevelEventEvent::ParticleDestroy,
                position: Vec3F {
                    x: x as f32 + 0.5,
                    y: y as f32 + 0.5,
                    z: z as f32 + 0.5,
