...
use super::xoroshiro::{Xoroshiro128, JavaRandom};
...
impl VanillaGenerator {
        ...
        if world_y == surface_height - 1 {
            return match biome {
                ...
                Biome::Savanna => {
                    ...
                    let hash = (local_x as u32).wrapping_mul(31).wrapping_add(local_z as u32);
                    if hash % 7 == 0 {
                        *blocks::COARSE_DIRT
                    } else {
                        *blocks::GRASS_BLOCK
        ...
        for local_z in (1u8..15).step_by(5) {
            for local_x in (1u8..15).step_by(5) {
                ...
                match biome {
                    ...
                    Biome::Jungle => {
                        self.place_jungle_tree(chunk, local_x, height as i16, local_z)
                    }
                    Biome::DarkForest => {
                    ...
                    Biome::Swamp => {
                        self.place_swamp_oak(chunk, local_x, height as i16, local_z)
                    }
                    Biome::FlowerForest => {
    ...
    fn place_oak_tree(&self, chunk: &mut Chunk, x: u8, ground_y: i16, z: u8) {
        ...
        let height_seed = (self.seed.wrapping_add(x as i64 * 31).wrapping_add(z as i64 * 17)) as u32;
            ...
            for dx in -radius..=radius {
                for dz in -radius..=radius {
                    ...
                    if dx.abs() == radius && dz.abs() == radius {
                        ...
                        let corner_hash = ((height_seed as i16 + dx * 7 + dz * 13 + layer * 3) % 2) == 0;
    ...
    fn place_birch_tree(&self, chunk: &mut Chunk, x: u8, ground_y: i16, z: u8) {
        ...
        let height_seed = (self.seed.wrapping_add(x as i64 * 37).wrapping_add(z as i64 * 23)) as u32;
        ...
        for layer in 0i16..4 {
            ...
            for dx in -radius..=radius {
                for dz in -radius..=radius {
                    ...
                    if dx.abs() == radius && dz.abs() == radius {
                        let corner_hash = ((height_seed as i16 + dx * 5 + dz * 11 + layer * 7) % 2) == 0;
                        if corner_hash && layer < 2 {
                            continue;
                        }
                    }
    ...
    fn place_spruce_tree(&self, chunk: &mut Chunk, x: u8, ground_y: i16, z: u8) {
        ...
        let height_seed = (self.seed.wrapping_add(x as i64 * 41).wrapping_add(z as i64 * 29)) as u32;
        ...
        for layer in 0..=leaves_height {
            ...
            for dx in -current_radius..=current_radius {
                for dz in -current_radius..=current_radius {
                    ...
                    if current_radius > 0 && dx.abs() == current_radius && dz.abs() == current_radius {
                        continue;
                    }
        ...
        for dy in -3i16..2 {
            ...
            for dx in -(radius as i16)..=(radius as i16) {
                for dz in -(radius as i16)..=(radius as i16) {
                    ...
                    chunk.set_block(nx as u8, ground_y + trunk_height - 1 + dy, nz as u8, *blocks::JUNGLE_LEAVES);
                }
            }
        }
    }
        ...
        for dy in -2i16..2 {
            ...
            for dx in -(radius as i16)..=(radius as i16) {
                for dz in -(radius as i16)..=(radius as i16) {
                    ...
                    chunk.set_block(nx as u8, ground_y + trunk_height - 1 + dy, nz as u8, *blocks::DARK_OAK_LEAVES);
                }
            }
        }
    }
    ...
    fn place_acacia_tree(&self, chunk: &mut Chunk, x: u8, ground_y: i16, z: u8) {
        ...
        for dx in -2i16..=2 {
            for dz in -2i16..=2 {
                ...
                chunk.set_block(nx as u8, ground_y + trunk_height, nz as u8, *blocks::ACACIA_LEAVES);
            }
        }
        // Top layer
        for dx in -1i16..=1 {
            for dz in -1i16..=1 {
                ...
                let nz = z as i16 + dz;
                if nx >= 0 && nx < 16 && nz >= 0 && nz < 16 {
                    chunk.set_block(nx as u8, ground_y + trunk_height + 1, nz as u8, *blocks::ACACIA_LEAVES);
                }
            }
        }
    }
    ...
    fn place_swamp_oak(&self, chunk: &mut Chunk, x: u8, ground_y: i16, z: u8) {
        ...
        self.place_leaves(chunk, x, ground_y + trunk_height - 1, z, *blocks::OAK_LEAVES);
        ...
        for local_z in 0u8..16 {
            for local_x in 0u8..16 {
                ...
                for y in -60i16..80 {
                    ...
                    let variant_noise2 = self.detail_noise.sample(fx * 0.08 + 100.0, fy * 0.08, fz * 0.08 + 100.0);
        ...
        for local_z in 0u8..16 {
            for local_x in 0u8..16 {
                ...
                for y in -60i16..128 {
                    ...
                    let ore_noise2 = self.tree_noise.sample(fx * 0.2 + 50.0, fy * 0.2, fz * 0.2 + 50.0);
                    let ore_noise3 = self.tree_noise.sample(fx * 0.12 + 100.0, fy * 0.12 + 100.0, fz * 0.12);
                    
                    // Position-based hash for variety
                    let hash = ((world_x.wrapping_mul(1337) ^ world_z.wrapping_mul(7919) ^ (y as i32).wrapping_mul(13)) as u32) ^ (ore_seed as u32);
                    let hash_f = (hash % 1000) as f64 / 1000.0;
                    
                    // Coal ore: Y 5-128, very common
                    if y >= 5 && y <= 128 && ore_noise1 > 0.75 - (y as f64 / 300.0) {
                        let ore = if is_deepslate { *blocks::DEEPSLATE_COAL_ORE } else { *blocks::COAL_ORE };
                        chunk.set_block(local_x, y, local_z, ore);
                    }
                    // Iron ore: Y -60 to 64, common
                    else if y >= -60 && y <= 64 && ore_noise2 > 0.78 {
                        let ore = if is_deepslate { *blocks::DEEPSLATE_IRON_ORE } else { *blocks::IRON_ORE };
                        chunk.set_block(local_x, y, local_z, ore);
                    }
                    // Copper ore: Y -16 to 112, moderately common
                    else if y >= -16 && y <= 112 && ore_noise1 < -0.78 && ore_noise3 > 0.3 {
                        let ore = if is_deepslate { *blocks::DEEPSLATE_COPPER_ORE } else { *blocks::COPPER_ORE };
                        chunk.set_block(local_x, y, local_z, ore);
                    }
                    // Gold ore: Y -60 to 32, uncommon
                    else if y >= -60 && y <= 32 && ore_noise3 > 0.85 {
                        let ore = if is_deepslate { *blocks::DEEPSLATE_GOLD_ORE } else { *blocks::GOLD_ORE };
                        chunk.set_block(local_x, y, local_z, ore);
                    }
                    // Redstone ore: Y -60 to 16, moderately common deep
                    else if y >= -60 && y <= 16 && ore_noise2 < -0.78 {
                        let ore = if is_deepslate { *blocks::DEEPSLATE_REDSTONE_ORE } else { *blocks::REDSTONE_ORE };
                        chunk.set_block(local_x, y, local_z, ore);
                    }
                    // Lapis ore: Y -60 to 64, rare, peaks at Y=0
                    else if y >= -60 && y <= 64 && ore_noise3 < -0.88 && hash_f > 0.7 {
                        let ore = if is_deepslate { *blocks::DEEPSLATE_LAPIS_ORE } else { *blocks::LAPIS_ORE };
                        chunk.set_block(local_x, y, local_z, ore);
                    }
                    // Diamond ore: Y -60 to 16, very rare
                    else if y >= -60 && y <= 16 && ore_noise1 > 0.92 && ore_noise2 > 0.5 {
                        let ore = if is_deepslate { *blocks::DEEPSLATE_DIAMOND_ORE } else { *blocks::DIAMOND_ORE };
                        chunk.set_block(local_x, y, local_z, ore);
                    }
                    // Emerald ore: Y -16 to 320 in mountains only, extremely rare
                    else if y >= -16 && y <= 100 && ore_noise1 > 0.95 && ore_noise2 < -0.5 && ore_noise3 > 0.7 {
                        let biome = self.get_biome(world_x, world_z);
                        if matches!(biome, Biome::WindsweptHills | Biome::SnowyTaiga) {
                            let ore = if is_deepslate { *blocks::DEEPSLATE_EMERALD_ORE } else { *blocks::EMERALD_ORE };
                            chunk.set_block(local_x, y, local_z, ore);
                        }
                    }
                }
                ...
                for _ in 0..cave_count {
                    ...
                    if rng.next_int(4) == 0 {
                        ...
                        self.carve_cave_room(
                            ...
                            &mut rng, start_x, start_y, start_z
                        );
    ...
    fn carve_cave_tunnel(
        ...
        width: f32,
        mut yaw: f32, mut pitch: f32,
        start_idx: i32, end_idx: i32,
        height_ratio: f64,
    ) {
        use std::f64::consts::PI;
                            ...
                            if dx * dx + dz * dz < 1.0 {
                                for ly in (min_y..max_y).rev() {
                                    ...
                                    if dy > -0.7 && dx * dx + dy * dy + dz * dz < 1.0 {
                                        let current = chunk.get_block(lx as u8, ly as i16, lz as u8);
                                        // Only carve stone, dirt, grass - not water
                                        if current != *blocks::WATER && current != *blocks::BEDROCK {
                                            // Lava below Y=10
                                            if ly < 10 {
                                                chunk.set_block(lx as u8, ly as i16, lz as u8, *blocks::LAVA);
                                            } else {
                                                chunk.set_block(lx as u8, ly as i16, lz as u8, *blocks::AIR);
                                            }
                                        }
                                    }
                                }
        ...
        for cx in (chunk_x - range)..=(chunk_x + range) {
            for cz in (chunk_z - range)..=(chunk_z + range) {
                ...
                self.carve_ravine_tunnel(
                    ...
                    start_x, start_y, start_z,
                    width, yaw, pitch,
                    0, 0, 3.0  // Height ratio of 3 makes ravines tall/narrow
                );
            }
        }
    }
    ...
    fn add_structures(&self, chunk: &mut Chunk, chunk_x: i32, chunk_z: i32) {
        ...
        if v_pos.chunk_x == chunk_x && v_pos.chunk_z == chunk_z {
            ...
            if height > Self::SEA_LEVEL
                && height < 90
                && matches!(biome, Biome::Plains | Biome::Savanna | Biome::Taiga | Biome::Desert)
            {
        ...
        if i_pos.chunk_x == chunk_x && i_pos.chunk_z == chunk_z {
            ...
            if matches!(biome, Biome::SnowyTaiga | Biome::SnowyMountains) && height > Self::SEA_LEVEL {
            ...
            for dx in -radius..=radius {
                for dz in -radius..=radius {
                    ...
                    if dist_sq <= (radius * radius) as i64 as i8 {
                        ...
                        let is_edge = dist_sq >= ((radius - 1) * (radius - 1).max(0)) as i64 as i8 || dy == 0;
                        if is_edge {
