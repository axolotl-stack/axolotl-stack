...
impl BlazeDBProvider {
    ...
    pub fn open<P: AsRef<Path>>(path: P, config: Option<BlazeConfig>) -> StorageResult<Arc<Self>> {
        ...
        let write_offset = data_file.metadata()
            .map(|m| m.len())
            .unwrap_or(0);
    ...
    fn load_index(path: &Path) -> StorageResult<HashMap<u64, IndexEntry>> {
        ...
        file.read_to_end(&mut data).map_err(|e| StorageError::Io(e))?;
    ...
    fn rebuild_index(file: &File) -> StorageResult<HashMap<u64, IndexEntry>> {
        ...
        loop {
            ...
            file.read_exact(&mut size_buf).map_err(|e| StorageError::Io(e))?;
            let size = u32::from_le_bytes(size_buf);
            
            // Skip CRC
            file.seek(SeekFrom::Current(4)).map_err(|e| StorageError::Io(e))?;
            
            // Read coordinates
            let mut x_buf = [0u8; 4];
            let mut z_buf = [0u8; 4];
            let mut dim_buf = [0u8; 4];
            file.read_exact(&mut x_buf).map_err(|e| StorageError::Io(e))?;
            file.read_exact(&mut z_buf).map_err(|e| StorageError::Io(e))?;
            file.read_exact(&mut dim_buf).map_err(|e| StorageError::Io(e))?;
            ...
            if remaining > 0 {
                file.seek(SeekFrom::Current(remaining)).map_err(|e| StorageError::Io(e))?;
            }
        }
        ...
        loop {
            ...
            match tokio::time::timeout(
                ...
                Err(_) => {
                    ...
                    if !pending_writes.is_empty() && last_flush.elapsed().as_millis() >= provider.config.flush_interval_ms as u128 {
                        if let Err(e) = provider.flush_writes(&mut pending_writes) {
    ...
    fn flush_writes(&self, writes: &mut Vec<WriteRequest>) -> StorageResult<()> {
        ...
        for req in writes.drain(..) {
            ...
            index.insert(req.morton, IndexEntry {
                offset,
                size: total_size,
            });
        }
    ...
    fn read_chunk_at(&self, entry: IndexEntry) -> StorageResult<ChunkColumn> {
        ...
        file.seek(SeekFrom::Start(entry.offset)).map_err(|e| StorageError::Io(e))?;
        
        let mut header = [0u8; 24];
        file.read_exact(&mut header).map_err(|e| StorageError::Io(e))?;
        ...
        file.read_exact(&mut data).map_err(|e| StorageError::Io(e))?;
