...
Unastar should not claim broad vanilla compatibility until the gates below pass on the pinned Bedrock protocol version exposed through `jolyne::valentine` (`bedrock_1_26_44` today). The near-term target is **vanilla-compatible join, spawn, and basic creative interaction**, not full survival parity.
